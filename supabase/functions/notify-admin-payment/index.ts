...
function buildHtml(o: {
  ...
  amount: number
  currency: string
}, reviewUrl: string | null, hasReceiptImage: boolean): string {
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .header { background: #ea580c; padding: 20px 24px; }
    .header h1 { color: #fff; margin: 0; font-size: 18px; }
    .header p { color: #fed7aa; margin: 4px 0 0; font-size: 13px; }
    .body { padding: 24px; }
...
<body>
  <div class="card">
    ...
    <div class="body">
      ...
      <div class="row"><span class="label">Payment: </span><span class="value">${escapeHtml(o.method)}</span></div>
      <div class="row total-row"><span class="label" style="font-weight:700;color:#0f172a">Amount: </span><span class="value">${escapeHtml(formatCurrency(o.amount, o.currency))}</span></div>
    </div>
    ${hasReceiptImage ? `
    ...
    <p class="note">The customer uploaded a payment receipt. Please verify it${hasReceiptImage ? ' above' : ' (see attachment)'} and approve or reject it.</p>
    ${reviewUrl ? `<a class="cta" href="${escapeHtml(reviewUrl)}">Review &amp; Approve Payment →</a>` : ''}
  </div>
</body>
</html>`
}
...
serve(async (req) => {
  ...
  try {
    ...
    const body: PaymentNotifyRequest = await req.json()
    if (!body.order_id) {
      return jsonResponse(req, { success: false, error: 'order_id is required' }, 400)
    }
    ...
    const { data: order, error: orderError } = await serviceClient
      .from('orders')
      .select('id, order_number, customer_name, customer_phone, customer_id, guest_access_token_hash')
      .eq('id', body.order_id)
      .maybeSingle()
    if (orderError || !order) return jsonResponse(req, { success: false, error: 'Order not found' }, 404)

    // Ownership check: either a logged-in customer who owns the order, or a
    // guest presenting the order number + access token issued at checkout.
    ...
    const { data: payment, error: paymentError } = await serviceClient
      .from('payments')
      .select('id, method, amount, currency, receipt_image_url')
      .eq('order_id', order.id)
      .neq('method', 'CASH_ON_DELIVERY')
    ...
    try {
      ...
      if (file) {
        ...
        attachment = {
          filename: `receipt-${order.order_number}.${extension}`,
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: ADMIN_EMAIL,
        subject: `Payment receipt submitted for order ${order.order_number}`.slice(0, 200),
        html: buildHtml({
    ...
    console.error('[notify-admin-payment]', err)
