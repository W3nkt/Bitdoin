import type { Translations } from './en'

const lo: Translations = {
  appName: 'ບິດດອຍ',
  tagline: 'ຕະຫຼາດໜັງສືໃນ ສປປ ລາວ',

  nav: {
    trackOrder: 'ຕິດຕາມຄຳສັ່ງ',
    contacts: 'ຕິດຕໍ່',
    home: 'ໜ້າຫຼັກ',
    catalog: 'ໜັງສື',
    cart: 'ກະຕ່າ',
    orders: 'ຄຳສັ່ງຊື້',
    profile: 'ໂປຣໄຟລ໌',
    search: 'ຄົ້ນຫາ',
    support: 'ຊ່ວຍເຫຼືອ',
    signIn: 'ເຂົ້າສູ່ລະບົບ',
    signOut: 'ອອກຈາກລະບົບ',
    admin: 'ຜູ້ດູແລ',
    knowledge: 'ຄັງຄວາມຮູ້',
    about: 'ກ່ຽວກັບພວກເຮົາ',
    faq: 'ຄຳຖາມທີ່ພົບເລື້ອຍ',
  },

  contacts: {
    intro: 'ຕິດຕໍ່ແອັດມິນຜ່ານ WhatsApp, Messenger, ໂທ, ຫຼື ອີເມວ.',
    noPhone: 'ຍັງບໍ່ມີເບີໂທ',
  },

  home: {
    hero: 'ຄົ້ນຫາໜັງສືຈາກຮ້ານທົ່ວ ສປປ ລາວ',
    heroSub: 'ປຽບທຽບລາຄາ ສັ່ງຊື້ອອນລາຍ ສົ່ງຮອດໜ້າເຮືອນ',
    searchPlaceholder: 'ຄົ້ນຫາຊື່ໜັງສື, ຜູ້ຂຽນ, ຫຼື ISBN…',
    featured: 'ໜັງສືເດັ່ນ',
    trending: 'ໜັງສືຮ້ອນ',
    forYou: 'ແນະນຳສຳລັບທ່ານ',
    categories: 'ໝວດໝູ່',
    recentlyViewed: 'ເບິ່ງລ່າສຸດ',
    viewAll: 'ເບິ່ງທັງໝົດ',
    emptyBooks: 'ຍັງບໍ່ມີໜັງສື',
    quotes: {
      thousandLives: 'ນັກອ່ານໄດ້ມີຊີວິດນັບພັນຊີວິດ ກ່ອນທີ່ຈະຈາກໄປ.',
      portableMagic: 'ໜັງສືແມ່ນເວດມົນທີ່ພົກພາໄດ້.',
    },
  },

  catalog: {
    title: 'ໜັງສືທັງໝົດ',
    filters: 'ການກັ່ນຕອງ',
    clearFilters: 'ລ້າງການກັ່ນຕອງ',
    sortBy: 'ຈັດລຽງຕາມ',
    sortOptions: {
      priceAsc: 'ລາຄາ: ຕ່ຳ → ສູງ',
      priceDesc: 'ລາຄາ: ສູງ → ຕ່ຳ',
      newest: 'ໃໝ່ສຸດ',
      popular: 'ຍອດນິຍົມ',
    },
    noResults: 'ບໍ່ພົບໜັງສືທີ່ກົງກັບການຄົ້ນຫາ',
    showing: 'ພົບ {{count}} ລາຍການ',
    sortTitle: 'ຊື່ A-Z',
    clearSearch: 'ລ້າງການຄົ້ນຫາ',
  },

  book: {
    addToCart: 'ໃສ່ກະຕ່າ',
    buyNow: 'ຊື້ທັນທີ',
    outOfStock: 'ໝົດສາງ',
    available: 'ມີສາງ',
    lowStock: 'ສາງໃກ້ໝົດ',
    priceFrom: 'ເລີ່ມຕົ້ນ',
    compareStores: 'ປຽບທຽບລາຄາຮ້ານ',
    estimatedDelivery: 'ກຳນົດສົ່ງ',
    pages: 'ຈຳນວນໜ້າ',
    language: 'ພາສາ',
    publisher: 'ສຳນັກພິມ',
    published: 'ວັນພິມ',
    isbn: 'ISBN',
    category: 'ໝວດໝູ່',
    description: 'ລາຍລະອຽດ',
    relatedBooks: 'ໜັງສືທີ່ກ່ຽວຂ້ອງ',
    notFound: 'ບໍ່ພົບໜັງສື',
    margin: 'ກຳໄລ',
  },

  cart: {
    title: 'ກະຕ່າສິນຄ້າ',
    empty: 'ກະຕ່າຍັງຫວ່າງ',
    continueShopping: 'ຊື້ຕໍ່ໄປ',
    subtotal: 'ລວມ',
    deliveryFee: 'ຄ່າສົ່ງ',
    deliveryFeeNote: 'ຄ່າຂົນສົ່ງຈ່າຍປາຍທາງ',
    checkout: 'ຊຳລະສິນຄ້າ',
    remove: 'ລຶບ',
    saveForLater: 'ເກັບໄວ້ກ່ອນ',
    qty: 'ຈຳນວນ',
    itemsFrom: 'ສິນຄ້າຈາກ {{count}} ຮ້ານ',
  },

  checkout: {
    orderComplete: 'ສັ່ງຊື້ສຳເລັດ',
    choosePayment: 'ເລືອກວິທີຊຳລະ',
    completePayment: 'ຊຳລະ ແລະ ສົ່ງຫຼັກຖານ',
    stepDelivery: 'ຈັດສົ່ງ',
    stepMethod: 'ວິທີຊຳລະ',
    stepPay: 'ຊຳລະ/ອັບໂຫລດ',
    loadingPayment: 'ກຳລັງກຽມຂໍ້ມູນການຊຳລະ…',
    orderSaved: 'ບັນທຶກຄຳສັ່ງແລ້ວ. ກະລຸນາລອງໂຫລດຂໍ້ມູນການຊຳລະອີກຄັ້ງ.',
    title: 'ກວດສອບ & ສັ່ງຊື້',
    deliveryInfo: 'ຂໍ້ມູນການຈັດສົ່ງ',
    fullName: 'ຊື່ ແລະ ນາມສະກຸນ',
    phone: 'ເບີໂທລະສັບ (WhatsApp)',
    phoneInvalid: 'ກະລຸນາປ້ອນເບີໂທ 8 ໂຕເລກໃຫ້ຖືກຕ້ອງ',
    logisticsProvider: 'ບໍລິສັດຂົນສົ່ງ',
    province: 'ແຂວງ',
    district: 'ເມືອງ',
    selectLogistics: 'ເລືອກບໍລິສັດຂົນສົ່ງ',
    selectProvince: 'ເລືອກແຂວງ',
    selectDistrict: 'ເລືອກເມືອງ',
    addressDetail: 'ບ້ານ ຫຼື ສາຂາ ບໍລິສັດຂົນສົ່ງ',
    address: 'ທີ່ຢູ່ຈັດສົ່ງ',
    notes: 'ໝາຍເຫດ (ຖ້າມີ)',
    paymentMethod: 'ວິທີຈ່າຍ',
    paymentMethods: {
      QR_PAYMENT: 'ສະແກນ QR code (OnePay)',
      BANK_TRANSFER: 'ໂອນເງິນຜ່ານທະນາຄານ',
      CASH_ON_DELIVERY: 'ຈ່າຍເງິນສົດ',
    },
    orderSummary: 'ສະຫຼຸບຄຳສັ່ງ',
    placeOrder: 'ສົ່ງຄຳສັ່ງ',
    orderPlaced: 'ສັ່ງຊື້ສຳເລັດ! #{{orderNumber}}',
    total: 'ລວມທັງໝົດ',
    confirmOrder: 'ຢືນຢັນ',
    changePaymentMethod: 'ປ່ຽນວິທີຊຳລະ',
  },

  payment: {
    payNow: 'ຊຳລະຄຳສັ່ງນີ້',
    payThenUpload: 'ສະແກນ QR ຫຼື ໂອນເງິນ ແລ້ວອັບໂຫລດຫຼັກຖານຢູ່ດ້ານລຸ່ມ.',
    submitProof: 'ອັບໂຫລດຫຼັກຖານການຊຳລະ',
    submitProofNote: 'ຄຳສັ່ງຈະຖືກສົ່ງໃຫ້ແອັດມິນກວດຫຼັງຈາກອັບໂຫລດ.',
    uploadAndSubmit: 'ອັບໂຫລດ ແລະ ສົ່ງໃຫ້ແອັດມິນ',
    downloadQr: 'ບັນທຶກຮູບ QR',
    imageOnly: 'ກະລຸນາອັບໂຫລດຮູບ JPG, PNG ຫຼື WebP.',
    fileTooLarge: 'ຮູບຫຼັກຖານຕ້ອງບໍ່ເກີນ 10 MB.',
    accountUnavailable: 'ຍັງບໍ່ມີຂໍ້ມູນບັນຊີຊຳລະ. ກະລຸນາຕິດຕໍ່ພະນັກງານ.',
    qrTitle: 'ສະແກນ QR ເພື່ອຊຳລະ',
    qrInstructions: 'ໃຊ້ແອັບທະນາຄານສະແກນ QR ແລ້ວອັບໂຫລດຮູບຖ່າຍໃບຮັບໃຕ້ນີ້',
    qrCode: 'ລະຫັດ QR',
    uploadReceipt: 'ອັບໂຫລດຫຼັກຖານການຊຳລະ',
    receiptUploaded: 'ອັບໂຫລດແລ້ວ ລໍຖ້າກວດສອບ',
    verifying: 'ກຳລັງກວດສອບ…',
    verified: 'ຢືນຢັນການຊຳລະແລ້ວ!',
    rejected: 'ການຊຳລະຖືກປະຕິເສດ',
    requiresReview: 'ກຳລັງກວດສອບໂດຍທີມງານ — ຈະຢືນຢັນໃນໄວໆນີ້',
    transferTo: 'ໂອນໄປທີ່ເລກບັນຊີນີ້',
    amount: 'ຈຳນວນ',
    reference: 'ລະຫັດອ້າງອີງ',
    method: 'ວິທີຊຳລະ',
    bankTransferTitle: 'ຂໍ້ມູນການໂອນເງິນ',
    bankName: 'ທະນາຄານ',
    accountName: 'ຊື່ບັນຊີ',
    accountNumber: 'ເລກທີ່ບັນຊີ',
    rejectionReason: 'ເຫດຜົນ',
    uploadNew: 'ອັບໂຫລດຫຼັກຖານໃໝ່',
    receipt: 'ໃບຮັບເງິນ',
    receiptNumber: 'ໃບຮັບ #',
    billTo: 'ໃບເກັບເງິນ',
    paymentDetails: 'ລາຍລະອຽດການຊຳລະ',
    sender: 'ຜູ້ໂອນ',
    bankNameLabel: 'ທະນາຄານ',
    transferDate: 'ວັນທີໂອນ',
    downloadReceipt: 'ບັນທຶກໃບຮັບເງິນ',
    trackingNote: 'ໃຊ້ລະຫັດນີ້ຕິດຕາມການຈັດສົ່ງ',
  },

  tracking: {
    title: 'ຕິດຕາມຄຳສັ່ງຊື້',
    subtitle: 'ໃສ່ລະຫັດຄຳສັ່ງ ແລະ ເບີໂທທີ່ໃຊ້ສັ່ງຊື້.',
    orderCode: 'ລະຫັດຄຳສັ່ງ',
    trackButton: 'ຕິດຕາມຄຳສັ່ງ',
    notFound: 'ບໍ່ພົບຄຳສັ່ງ',
    checkDetails: 'ກວດລະຫັດ ແລະ ເບີໂທ ແລ້ວລອງໃໝ່.',
    saveCode: 'ບັນທຶກລະຫັດນີ້ເພື່ອຕິດຕາມ',
    copyCode: 'ສຳເນົາລະຫັດ',
    codNote: 'ຊຳລະເງິນເມື່ອສິນຄ້າສົ່ງຮອດ.',
    delivery: 'ຂໍ້ມູນຈັດສົ່ງ',
    viewTracking: 'ເບິ່ງການຕິດຕາມ',
  },

  orders: {
    title: 'ຄຳສັ່ງຊື້ຂອງຂ້ອຍ',
    empty: 'ທ່ານຍັງບໍ່ມີຄຳສັ່ງຊື້',
    orderNumber: 'ຄຳສັ່ງ #',
    placedOn: 'ສັ່ງເມື່ອ',
    status: 'ສະຖານະ',
    viewDetail: 'ເບິ່ງລາຍລະອຽດ',
    tracking: 'ຕິດຕາມ',
    courier: 'ຜູ້ສົ່ງ',
    trackingNumber: 'ລະຫັດຕິດຕາມ',
    estimatedDelivery: 'ກຳນົດສົ່ງ',
    notFound: 'ບໍ່ພົບຄຳສັ່ງຊື້',
    items: 'ລາຍການ',
    manageAll: 'ເບິ່ງຄຳສັ່ງຊື້ທັງໝົດຂອງທ່ານ',
    receiptReady: 'ໃບຮັບເງິນຂອງທ່ານພ້ອມແລ້ວ',
    trackWithReceipt: 'ຕິດຕາມຄຳສັ່ງດ້ວຍໃບຮັບ',
    awaitingPayment: 'ລໍຖ້າການຊຳລະ',
    underReview: 'ກຳລັງກວດສອບການຊຳລະ',
    paymentRejected: 'ການຊຳລະຖືກປະຕິເສດ',
  },

  auth: {
    signIn: 'ເຂົ້າສູ່ລະບົບ',
    signUp: 'ສ້າງບັນຊີ',
    emailMethod: 'ອີເມລ',
    phoneOtpMethod: 'OTP ໂທລະສັບ',
    phone: 'ເບີໂທລະສັບ',
    email: 'ອີເມລ',
    password: 'ລະຫັດຜ່ານ',
    passwordPlaceholder: 'ຢ່າງໜ້ອຍ 8 ຕົວອັກສອນ',
    passwordDots: '••••••••',
    emailPlaceholder: 'you@example.com',
    namePlaceholder: 'ຊື່ຂອງທ່ານ',
    signInWithPhone: 'ເຂົ້າລະບົບດ້ວຍເບີໂທ',
    codeSentTo: 'ສົ່ງລະຫັດໄປທີ່ {{phone}}',
    accountCreated: 'ສ້າງບັນຊີແລ້ວ! ຕອນນີ້ທ່ານສາມາດເຂົ້າລະບົບໄດ້.',
    otpSent: 'ສົ່ງ OTP ແລ້ວ!',
    otp: 'ໃສ່ລະຫັດ OTP',
    sendOtp: 'ສົ່ງ OTP',
    verifyOtp: 'ຢືນຢັນ',
    continueWithGoogle: 'ສືບຕໍ່ດ້ວຍ Google',
    continueWithFacebook: 'ສືບຕໍ່ດ້ວຍ Facebook',
    or: 'ຫຼື ເຂົ້າສູ່ລະບົບດ້ວຍ',
    name: 'ຊື່ ແລະ ນາມສະກຸນ',
    haveAccount: 'ມີບັນຊີແລ້ວ?',
    noAccount: 'ຍັງບໍ່ມີບັນຊີ?',
  },

  sidebar: {
    browse: 'ເລືອກເບິ່ງ',
    bestSeller: 'ຂາຍດີ',
    justArrived: 'ມາໃໝ່',
    editorPicks: 'ຄັດເລືອກ',
    allBooks: 'ໜັງສືທັງໝົດ',
    category: 'ໝວດໝູ່',
    language: 'ພາສາ',
    lao: 'ລາວ',
    english: 'ອັງກິດ',
    thai: 'ໄທ',
    chinese: 'ຈີນ',
    french: 'ຝຣັ່ງ',
    note: 'ປຽບທຽບລາຄາຈາກຮ້ານໜັງສື ສັ່ງຄັ້ງດຽວ ແລ້ວໃຫ້ ບິດດອຍ ປະສານງານສ່ວນທີ່ເຫຼືອ.',
  },

  profile: {
    preferences: 'ການຕັ້ງຄ່າ',
    currency: 'ສະກຸນເງິນ',
    managePlatform: 'ຈັດການແພລດຟອມ',
    subscriptionDetails: 'Premium Services',
    subscriptionSubtitle: 'Plans, benefits, and member access',
    subscriptionDashboard: 'Subscription Dashboard',
    subscriptionDashboardSubtitle: 'Manage Bitdoin Premium',
    bookstoreDashboard: 'BookStore Dashboard',
    bookstoreDashboardSubtitle: 'Manage bookstore operations',
    premiumDashboard: 'Premium Dashboard',
    premiumDashboardSubtitle: 'Manage Bitdoin Premium',
    changePhoto: 'Change profile picture',
    changeCover: 'Change cover',
    imageOnly: 'Please upload a JPG, PNG, or WebP image.',
    imageTooLarge: 'The image must be 5 MB or smaller.',
    uploadFailed: 'Unable to update your profile image.',
    uploadSuccess: 'Profile image updated.',
    cropPhoto: 'Crop profile picture',
    cropPhotoHint: 'Drag the circle to frame your profile picture before saving.',
    savePhoto: 'Save photo',
  },

  subscription: {
    title: 'Subscription Details',
    comingSoon: 'Your subscription details page will be available here later.',
  },

  quotes: {
    specialDay: 'ມື້ພິເສດ',
    quoteLabel: 'ຄຳຄົມ {{number}}',
    previous: 'ຄຳຄົມກ່ອນໜ້າ',
    next: 'ຄຳຄົມຖັດໄປ',
  },

  knowledge: {
    title: 'Knowledge Hub',
    titleLo: 'ຄັງຄວາມຮູ້',
    tagline: 'ຄຳຄົມ ບົດຄວາມ ແລະ ປັນຍາ ສຳລັບນັກຮຽນ ແລະ ຜູ້ຮັກການຮຽນຮູ້.',
    allCategories: 'ທຸກຫົວຂໍ້',
    allTypes: 'ທຸກປະເພດ',
    biographies: 'ຊີວະປະຫວັດ',
    quotes: 'ຄຳຄົມ',
    articles: 'ບົດຄວາມ',
    tips: 'ຄຳແນະນຳ',
    blogs: 'ບລັອກ',
    contentType: 'ປະເພດເນື້ອຫາ',
    clearFilters: 'ລ້າງຕົວກອງ',
    results: 'ຜົນລັບ',
    result: 'ຜົນລັບ',
    filterLabel: 'ກອງຂໍ້ມູນ',
    featured: 'ເດັ່ນ',
    readMore: 'ອ່ານຕໍ່',
    by: 'ໂດຍ',
    backToHub: 'ກັບຫາຄັງຄວາມຮູ້',
    previousPost: 'ກ່ອນໜ້າ',
    nextPost: 'ຕໍ່ໄປ',
    relatedPosts: 'ອ່ານເພີ່ມເຕີມ',
    noPostsFound: 'ບໍ່ພົບເນື້ອຫາໃນຫົວຂໍ້ນີ້.',
    searchPlaceholder: 'ຄົ້ນຫາບົດຄວາມ ແລະ ຄຳຄົມ…',
    minRead: 'ນາທີ ອ່ານ',
    published: 'ເຜີຍແຜ່',
    views: 'ຄັ້ງ',
    adminTitle: 'ຈັດການຄັງຄວາມຮູ້',
    addPost: 'ເພີ່ມໂພສ',
    editPost: 'ແກ້ໄຂໂພສ',
    deletePost: 'ລຶບໂພສ',
    confirmDelete: 'ທ່ານຕ້ອງການລຶບໂພສນີ້ບໍ?',
    publishToggle: 'ສະຫຼັບສະຖານະເຜີຍແຜ່',
    featureToggle: 'ສະຫຼັບສະຖານະເດັ່ນ',
    formTitle: 'ຫົວຂໍ້ (ອັງກິດ)',
    formTitleLo: 'ຫົວຂໍ້ (ລາວ)',
    formContent: 'ເນື້ອຫາ (ອັງກິດ)',
    formContentLo: 'ເນື້ອຫາ (ລາວ)',
    formExcerpt: 'ສາລະສັງລວມ (ອັງກິດ)',
    formExcerptLo: 'ສາລະສັງລວມ (ລາວ)',
    formAuthor: 'ຜູ້ຂຽນ',
    formType: 'ປະເພດ',
    formCategory: 'ໝວດໝູ່',
    formTags: 'ແທັກ (ຄັ່ນດ້ວຍເຄື່ອງໝາຍ,)',
    formFeatured: 'ໂພສເດັ່ນ',
    formPublished: 'ເຜີຍແຜ່',
    saveDraft: 'ບັນທຶກເປັນແບບຮ່າງ',
    publish: 'ບັນທຶກ ແລະ ເຜີຍແຜ່',
  },

  common: {
    loading: 'ກຳລັງໂຫລດ…',
    error: 'ເກີດຂໍ້ຜິດພາດ',
    retry: 'ລອງໃໝ່',
    save: 'ບັນທຶກ',
    cancel: 'ຍົກເລີກ',
    delete: 'ລຶບ',
    edit: 'ແກ້ໄຂ',
    add: 'ເພີ່ມ',
    search: 'ຄົ້ນຫາ',
    view: 'ເບິ່ງ',
    close: 'ປິດ',
    confirm: 'ຢືນຢັນ',
    back: 'ກັບຄືນ',
    next: 'ຕໍ່ໄປ',
    yes: 'ແມ່ນ',
    no: 'ບໍ່',
    all: 'ທັງໝົດ',
    active: 'ໃຊ້ງານ',
    inactive: 'ປິດ',
    unknown: 'ບໍ່ຮູ້',
    copied: 'ສຳເນົາແລ້ວ!',
    copy: 'ສຳເນົາ',
    lak: 'ກີບ',
    usd: 'ດອນ',
    required: 'ຕ້ອງລະບຸ',
    optional: 'ຖ້າມີ',
  },

  admin: {
    dashboard: 'ໜ້າຫຼັກ',
    books: 'ໜັງສື',
    bookstores: 'ຮ້ານໜັງສື',
    pricing: 'ລາຄາ',
    orders: 'ຄຳສັ່ງ',
    payments: 'ການຊຳລະ',
    deliveries: 'ການຈັດສົ່ງ',
    analytics: 'ການວິເຄາະ',
    notifications: 'ການແຈ້ງເຕືອນ',
    settings: 'ຕັ້ງຄ່າ',
    auditLog: 'ປະຫວັດການ',
    gmv: 'ມູນຄ່າສິນຄ້າລວມ',
    revenue: 'ລາຍໄດ້',
    margin: 'ກຳໄລ',
    pendingPayments: 'ລໍຖ້າກວດການຊຳລະ',
    pendingDeliveries: 'ລໍຖ້າຈັດສົ່ງ',
    topBooks: 'ໜັງສືຂາຍດີ',
    addBook: 'ເພີ່ມໜັງສື',
    addBookstore: 'ເພີ່ມຮ້ານ',
    importCsv: 'ນຳເຂົ້າ CSV',
    verifyPayment: 'ຢືນຢັນການຈ່າຍ',
    rejectPayment: 'ປະຕິເສດ',
    assignCourier: 'ກຳນົດຜູ້ສົ່ງ',
    updateTracking: 'ອັບເດດສະຖານະ',
    sendWhatsApp: 'ສົ່ງ WhatsApp',
    marginRule: 'ກົດລະບຽບກຳໄລ',
    globalMargin: 'ກຳໄລລ່ວງໜ້າ',
  },

  landing: {
    eyebrow: 'ບັນຊີດຽວ, ສອງແພລດຟອມ',
    title: 'ທ່ານຢາກໄປທາງໃດ?',
    subtitle: 'ບິດດອຍລວມຮ້ານໜັງສືທົ່ວປະເທດ ແລະ ແພລດຟອມພັດທະນາຕົນເອງໄວ້ນຳກັນ — ເລືອກໜຶ່ງເພື່ອເລີ່ມຕົ້ນ.',
    bookstore: {
      badge: 'ຕະຫຼາດ',
      title: 'ຮ້ານໜັງສືບິດດອຍ',
      description: 'ປຽບທຽບລາຄາຈາກຮ້ານໜັງສືທົ່ວ ສປປ ລາວ ສັ່ງຊື້ອອນລາຍ ແລະ ຮັບການສົ່ງເຖິງໜ້າເຮືອນ.',
      feature1: 'ປຽບທຽບລາຄາຫຼາຍຮ້ານ',
      feature2: 'ສັ່ງຊື້ອອນລາຍ ຈ່າຍໄດ້ຫຼາຍຊ່ອງທາງ',
      feature3: 'ຕິດຕາມການສົ່ງແບບເລຍທັນທີ',
      cta: 'ເບິ່ງໜັງສື',
    },
    premium: {
      badge: 'ສະມາຊິກ',
      title: 'ບິດດອຍ ອາຄາເດມີ',
      description: 'ແພລດຟອມການຮຽນຮູ້ພ້ອມໂຄ້ດສ່ວນຕົວ ຄວາມທ້າທາຍປະຈຳວັນ ແລະ ການຕິດຕາມນິໄສ ເພື່ອຊ່ວຍໃຫ້ທ່ານເຕີບໂຕ.',
      feature1: 'ໂຄ້ດການຮຽນຮູ້ສ່ວນຕົວ',
      feature2: 'ຄວາມທ້າທາຍປະຈຳວັນ ແລະ ຕິດຕາມນິໄສ',
      feature3: 'ອັນດັບຊຸມຊົນ ແລະ ລາງວັນ',
      cta: 'ສຳຫຼວດອາຄາເດມີ',
    },
    footerNote: 'ຍັງບໍ່ແນ່ໃຈບໍ? ທ່ານສາມາດສະລັບລະຫວ່າງແພລດຟອມໄດ້ພາຍຫຼັງ.',
  },
}

export default lo
